...
> **✅ Status: Phase 7 (RF/EM cross-domain integration) complete — all 16 plugins implemented.** The
> marketplace registry, per-plugin manifests, CI, and the shared `design_state.json` schema are in
> place. The **core spine** (Phase 1: circuit, simulation, infrastructure, meta), the **HDL/AMS
> milestone** (Phase 2: architecture, modeling, ams-verification), the **physical tier** (Phase 3:
...
> `analog-design-reliability` and `analog-design-characterization`), the **RF emphasis tier**
> (Phase 5: `analog-design-rf` and `analog-design-em`), and the **mixed-signal top** (Phase 6:
> `analog-design-ams-integration`) are all fully implemented (detailed stage rules, memory wiring,
> fix_request loop including AMS→modeling, DRC/LVS→layout, EM/IR→layout / ESD→circuit, and
> ams-integration→layout/circuit/modeling routing). Phase 6 also delivers one-step installers
> (`install.sh` / `install.ps1`), multi-IDE export (`ides/`, generated by `tools/export_ides.py`),
> QoR trend tracking (`tools/qor_trends.py`), and a tagged release workflow
> (`.github/workflows/release.yml`). **Phase 7** wires the RF emphasis tier into the cross-domain
> loop: `rf-design` is now a producer (after its stage-local caps it opens a `fix_request` →
> `circuit-design` for device rework or → `em-modeling` for a passive shortfall), `em-modeling`
> services those passive re-solves, and re-validation routes back to `rf-design`. The remaining
> deferred enhancements (end-to-end validation harness, deeper tool/PDK coverage) are tracked in
> [`FUTURE_WORK.md`](FUTURE_WORK.md). See [`PLAN.md`](PLAN.md) for the roadmap; the **Phase** column
> below marks each domain's delivery phase (all implemented).
